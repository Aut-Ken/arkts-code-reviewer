...
截至 2026-07-16、提交 `a83eeb6` 的可验证基线：

```text
主项目测试                   1196 passed / 3 skipped；skip 为 PostgreSQL/FastEmbed 可选集成
...
Tag Truth governance        通用选样、双审、consensus、Git seal、近重复筛查、校准与 publication 合同已实现；
                            无真实 Pair Truth/盲测 campaign，Evidence 仍为 not_qualified
...
通用 Tag Truth 治理链已经进一步覆盖：

```text
PairSelection
-> path-redacted PairReviewPacket
-> 两份独立人工 ReviewReceipt
-> PairConsensus
...
该链冻结样本泄漏 component、双审原始票、Oracle 方向/聚合语义、80 个 duplicate component、
80 个 independent component、每个二元指标 Pair 独占一个 component，以及
freeze/release/report/approval 的完整 identity。`passed` 只表示可以进入人工 policy review，
不表示 approved 或 Evidence qualified；self-hash 只证明内容 identity，不能替代从完整上游
artifact 重建。该重建以调用方提供且 self-hash 有效的 PairSelection 为根，只验证 typed artifact
chain，不验证 Pair source Git provenance 或声明 verifier closure 的真实 blob bytes。当前仓库
没有真实 PairSelection、双审 Receipt、Consensus、CalibrationReport 或 ApprovalReceipt，也没有
policy/screening v2。
...
当前优先工作切换为 `TAG-CORPUS-01`：固定
`/home/autken/Code/applications_app_samples` 的实时 revision，对全部允许范围内的 `.ets/.ts`
代码做只读事实统计，识别现有 24 个 Active Tag 的覆盖缺口并形成候选 Tag 清单。该阶段只产生
候选、样本统计和触发器方案，不自动修改 `tags.yaml`；每个候选必须单独经过人工语义确认、
正例/hard-negative Truth、shadow 运行和量化门禁，才讨论 Draft/Active 配置迁移。

真实 Baselined 知识发布、Rules、Prompt/Final LLM 与结构化 Finding 仍是后续主链；Tag 候选发现
不能把 Retrieval 命中或外部文档关键词直接当作代码事实。
...
| 10 | 评测与反馈闭环 | `partial`（治理合同和合成门禁已实现；无真实 Tag/Pair/Finding truth） | [10-evaluation-feedback.md](modules/10-evaluation-feedback.md) |
