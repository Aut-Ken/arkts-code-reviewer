...
显式 FR-02 candidate 使用 `tag-config-v3/feature-routing-v2` 保存
`any_symbol_leaf` normalized provenance，但默认 `tags-v1`、正式 v1 Golden、
QuestionBinding 和下游消费链均未切换。`tag-config-v1/v2` 和 `feature-routing-v1`
的既有合同继续冻结，该 candidate 也尚未获得生产激活资格。
