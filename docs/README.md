...
ReviewUnit                  RU-0～RU-4 已完成；v2 Golden 16/16，v1 兼容门禁仍为 14/16
...
RU-4 已建立相互独立的 14-case ChangeSet Golden 和 16-case ReviewUnit v2 Golden；RU-5 先建立
12～16 个 Context Golden case，再实现关系、分组和预算。
...
和 GitCode 回写均是下游模块，不得为了完成 RU-5 提前混入。下一项是 RU-5 的有界关联
上下文、分组和真实 code-context budget；RU-4 没有接入 Git/GitCode，也没有改变 Parser、
Tagger 或 Retrieval 的事实语义。
