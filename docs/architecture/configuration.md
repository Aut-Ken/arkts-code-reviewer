...
Knowledge Clause 主链的三份配置分别固定首批 seed、确定性 annotation 和 Grok 审核外发边界。
Document-First 的三份配置分别固定单文档 DeepSeek smoke 外发策略、7 文档 pilot selection 和
对应的精确外发策略。Rules、Reviewer、Output、Evaluation 配置仍是目标设计，当前仓库没有对应
运行时 loader。`tagger.py` 只保留
...
### 2.1 Document-First 配置边界

`config/knowledge_document_card_export.yaml` 是已执行过一次的单文档 development smoke 策略，
只允许固定 revision 上的 `taskpool-vs-worker.md`、`deepseek-v4-pro` 和
`deepseek-document-card-v1`。它冻结完整正文、请求大小、响应体与输出 token 上限、最多一次
attempt、无重试、TLS 校验、禁用 redirect 和 `trust_env`。它是静态外发锁，不等于 live 执行
批准；live runner 还要求精确 Plan ID、wire body SHA-256、最大输出 token 和固定
acknowledgement。

`config/knowledge_document_card_pilot_selection_v1.yaml` 选择 7 篇 Registry + Knowledge Seed 中的
固定 Markdown，qualification 明确是 `pilot_selection_not_export_or_execution_authorization`。
`config/knowledge_document_card_pilot_export_v1.yaml` 为这 7 个精确 source/revision/path 提供独立
allowlist 和单 Plan 上限；它同样不是用户执行批准。

当前由这两份 pilot 配置生成的离线 inspect 固定为：
...
Aggregate caps:
  ...
  14000000 response bytes / 840000 ms
```

`tools/prepare_deepseek_document_card_campaign.py` 只有 inspect/materialize 路径：不会读取
`DEEPSEEK_API_KEY`、不会构造 transport、不会访问网络，并固定
`execution_authorized=false`。`tools/run_deepseek_document_card_campaign.py` 的默认路径同样只重建
Campaign 并逐字节核对已落盘的 selection、inspection 和每个 Plan 的 `00`～`05`；它不会读取
credential。真实执行必须同时提供 `--execute-live`、`--allow-real-transport`、精确 Campaign ID、
Plan-set、文档数、五项聚合上限、固定确认语以及显式 state/output 目录，任一不匹配都在读取
`DEEPSEEK_API_KEY` 前拒绝。2026-07-22 已按这些精确控制执行一次真实 Campaign：7/7 Plan 均为
`valid_card`，共 7 次尝试、0 次重试、0 个失败；根回执为
`document-card-campaign-live-receipt:sha256:49e7609094eac9639c64ba29e9598e66fc3c143a8cf61c21636c66bcd5b88411`。
该历史运行不改变配置的外发 allowlist，也不构成以后再次执行的授权；7 个 replay marker 已消费，
不得重试。回执固定为 `navigation_only_not_evidence`、`production_qualified=false`，真实调用成功不
等于 Document Card 内容质量合格。

Document Card Prompt 位于 `prompts/knowledge/deepseek-document-card-v1.md`，其版本和 SHA-256
同时进入 Request/Plan identity。配置只允许模型生成导航摘要；`DocumentCard` 和
`DocumentCatalogBuild` 固定 `evidence_eligible=false`，配置不能把它们提升为 Clause、
KnowledgeIndex 或 EvidencePack。
...
| `ARKTS_DOCUMENT_PROJECTION_DATABASE_URL` | 无；可回退 `DATABASE_URL` | 独立 L2 `document_projection` schema 的 migration 连接串 |

`arkts-retrieval publish` 还允许通过显式参数覆盖数据库 URL、模型 ID、维度、cache 和
`local-files-only`；模型 ID 不属于 `retrieval.yaml`，凭据不得写入配置或构建产物。

`arkts-document-projection-db` 只执行 checksum-protected PostgreSQL migration。连接串优先级为
显式 `--database-url`，然后 `ARKTS_DOCUMENT_PROJECTION_DATABASE_URL`，最后 `DATABASE_URL`。
该命令只建立隔离 Shadow schema，不会生成 DeepSeek Mapping、Grok 审核结果、L2 文档资产，
也不会把任何 Projection 发布为 Evidence。

### Knowledge Source Registry 与 Document Card

| 变量 | 默认值 | 用途 |
|---|---|---|
| `ARKTS_SOURCE_REGISTRY` | `/home/autken/Code/arkts-knowledge/registry/sources.yaml` | 未显式传入路径时选择 Source Registry |
| Registry 中每项 `env_override` 指定的变量 | 对应 `local_path` | 覆盖该来源的本地 checkout；必须是绝对路径且仍通过 Git identity 校验 |
| `DEEPSEEK_API_KEY` | 无 | 仅受控 Document Card live runner 执行前通过 credential provider 读取；inspect-only Campaign 不读取 |
...
主项目的 `resolve_source_path()` 已读取这些变量，解析顺序是：
...
环境变量覆盖值必须是绝对路径。`verify_source_checkout()` 随后校验实际路径等于 Git toplevel，
且 remote、branch 和 `HEAD` 与 Registry 登记一致；覆盖路径不会绕过固定 revision。Registry 文件
本身由 `load_source_registry()` 从显式绝对路径读取，未显式传入时才使用
`ARKTS_SOURCE_REGISTRY`，再回退到默认路径。
