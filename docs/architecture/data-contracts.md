...
## 5. CodeSourceRef 与目标 ChangeSet

RU-3 已实现 `CodeSourceRef`，用于把 Parser/ReviewUnit 结果绑定到不可变源码快照。RU-4 将在
此基础上增加 `ChangeAtom/ChangeSet`；当前 `FileInput/FileHunk` 尚未实现精确 diff 合同。
...
`context_span` 从文件源码切片，`changed_new_lines` 使用 1-based 文件绝对行。RU-3 主链为
非 fallback Unit 填充 `source_ref_id` 与 tagged `owner_ref`；fallback 不伪造 owner。
...
`source_ref_id` 已在 RU-3 引入，`change_atom_ids` 将在 RU-4 引入；兼容对象仍保留
...
`review-unit-build-v2`；RU-4 再增加顶层 `change_set_id` 并填后者。diff 文件无 hunk、hunk 越界或
粗粒度 hunk 有未归属行时，必须通过这个信封显式表达，不能静默改为 full
review。

`review-unit-build-v1` 继续作为 RU-2 兼容 schema；RU-3 主链输出
`review-unit-build-v2`。其中 `parser_quality` 使用 `parser_layer` 和排序去重后的 `warnings`，
