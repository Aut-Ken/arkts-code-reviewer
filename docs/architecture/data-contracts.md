...
transport；调用方必须分别显式 opt-in 对应 transport 类别，否则保持零 dispatch：
...
`verify_ai_tag_shadow_campaign_execution_result(...)` 有两个明确分层：
...
目录/`0600` 文件原子消费一次本地 reservation marker；同一 state directory 不自动 retry/resume/reset。
终端 summary 不输出代码、路径/符号、Prompt、wire body、raw response、模型 reason、credential 或 state path。
