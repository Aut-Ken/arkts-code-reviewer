...
## 11. Unit Feature Context 与 Feature Routing v1 / candidate v2
...
只有显式注入 `tag-config-v3` 的 FR-02 影子评估才输出
`feature-routing-v2`。该 candidate output 当前不进入默认 `CodeAnalyzer`、
...
| `diagnostics` | 当前只能来自 `UnitFactScope.unit_owner_unresolved` |
...
`validate_replay(scopes, config)` 会重新路由并要求对象完全相等。模型构造器自身只证明图内部
