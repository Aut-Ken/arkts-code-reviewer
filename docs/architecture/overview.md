...
| 评测闭环 | `partial` | Parser 至 Retrieval 的模块 Golden 已存在；通用 Tag Truth 选样、双审、seal、近重复筛查、校准和 publication 合同已实现；无真实 Pair/Tag/Finding Truth |
...
Tag Truth governance           selection/review/consensus/provenance/near-duplicate/calibration/publication contracts implemented
Tag quality status             24 Active Tags；无新增 Active Tag；无真实通用 Tag P/R 或 production-prevalence proof
Near-duplicate status          synthetic contract tests passed；无真实 Pair Truth；Evidence not_qualified
```
...
### 4.3 Feature Routing 质量证明与评测治理边界

Feature Routing 的运行合同完成，不等于 Tag taxonomy 已覆盖真实 ArkTS 代码。当前必须分开报告：

```text
合同已实现
  24 个 Active Tag、12 个 Dimension、12 个 Review Question
  可重放 FeatureRoutingResult、Golden、wheel 配置和 fail-closed loader

合成/开发回归已通过
  16-case Feature Routing Golden perfect
  lifecycle/relational-store 候选有 development regression
  Tag Truth 双审、seal、近重复和校准治理代码有合成负向测试

真实质量尚未证明
  没有真实通用 Tag blind campaign
  没有真实 Pair Truth 与 calibrated near-duplicate policy
  没有 production-prevalence Tag Precision/Recall
```
...
PairSelection
-> PairReviewPacket
-> two independent ReviewReceipts
-> PairConsensus
-> exhaustive OraclePredictionSet
...
该链防止已识别的 leakage component 跨 split 或重复放大二元样本量，也阻止仅凭 self-hashed
Report 生成正式 approval。但 reviewer/custodian/approver identity、source Git provenance 和
verifier closure 仍未由外部系统认证，所以 Report/ApprovalReceipt 都固定保留
`evidence_qualification_status=not_qualified`。这里的 typed-artifact rebuild 从 self-hash
有效的 PairSelection 开始，不应被描述为 source checkout 或代码 closure 已验证。
...
Milestone 1g Tag Truth 双审、seal、近重复校准与 publication 治理   合同已实现；无真实 Pair/Tag qualification
...
当前优先执行 `TAG-CORPUS-01`：从 pinned `applications_app_samples` 只读统计真实代码特征，
形成候选 Tag、触发条件、Dimension/RQ 和 hard-negative 证据；它不自动修改默认配置。之后每个
候选单独经过 Truth、shadow 和门禁，再决定 Draft/Active。当前不应先做 Reranker、全量 Skills
导入、全量 XTS 索引或 GitCode 服务化。
