...
| ReviewUnit | `partial` | RU-0～RU-4 已完成：精确 ChangeSet、base/head parse-once 和 `review-unit-build-v3` 已实现；RU-5 待完成 |
...
当前 `AnalysisResult` 能输出 ChangeSet、FileAnalysis、ReviewUnit、UnitFactScope、兼容
RetrievalQuery 原料和 Parser metadata，不能输出正式代码评审 Finding。
...
仍以 `14/16` 作为冻结兼容目标，不回写 expected。
