...
只是兼容视图，后续检索必须消费 profile 的 `retrieval_dimensions/routing_dimensions`，不能用
MR 并集绕过 policy。
...
ArkTS 代码总体准确率为 100%。当前尚未实现 relation discovery、Knowledge、Retrieval、Rules
和 Prompt；这些模块必须消费 Feature 产物，不能反向改写其事实作用域和 policy。
