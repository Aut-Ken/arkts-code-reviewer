...
本文最初用于解释基础概念，其中部分样例沿用早期兼容模型。当前精确配置和运行时边界请以
[Tag 识别方法与完整路由表](../reference/feature-routing-tags-v1.md)、
[Dimension 推导方法与完整路由表](../reference/feature-routing-dimensions-v1.md) 和
[Feature Routing canonical 文档](../modules/04-feature-routing.md) 为准。

截至 2026-07-14，Parser L1、FileAnalysis、ReviewUnit、Feature Routing 与 Retrieval v1
核心链路已经实现并有独立 Golden；真实 Baselined 知识、自动 Supporting Context 发现以及
正式 LLM/Finding 闭环仍未完成。不要把“管道能运行”误读为“真实评审质量已经合格”。
...
## 7. 24 个 Tags 逐项解释（历史兼容视图）

> **注意：本节保留的是早期 `CodeFacts → Tagger` 教学视图，不再是当前运行时路由表。**
> 当前实现从 `UnitFactScope.unit_exact/file_hints` 分域匹配，且触发器比本节示意更严格。
> 请使用 [Tag 识别方法与完整路由表（tags-v1）](../reference/feature-routing-tags-v1.md)
> 查阅全部 24 个 Tag。尤其不要采用本节对任意 `.on/.off/.once`、任意 `onXxx` 或裸
> lifecycle symbol 的宽泛描述。

早期教学模型中，Tagger 输入聚合 `CodeFacts`，输出一个字符串集合。

Tag 只表示代码场景，不表示已经发现问题。

### 7.1 资源类 Tags

| Tag | 早期简化触发 | ArkTS 示例 | 为什么需要关注 | 早期示意维度 |
|---|---|---|---|---|
...
早期设计曾把下面这些 Tag 的关注点归入核心维度或 DIM-12；这不是当前正式映射。

| Tag | 早期简化触发 | ArkTS 示例 | 早期示意关注 |
|---|---|---|---|
...
## 8. 12 个 Dimensions 逐项解释（历史兼容视图）

> **注意：本节只帮助理解名称，不是当前 Dimension 路由合同。** 当前实现区分
> `dimensions`、`retrieval_dimensions`、`routing_dimensions` 和 Review Question；
> `always_check` 也不等于自动发起知识检索。准确算法和路由表见
> [Dimension 推导方法与完整路由表（dimensions-v1）](../reference/feature-routing-dimensions-v1.md)。
...
## 9. 用样例推导 Tags 和 Dimensions（历史教学推导）

> 本节基于早期聚合 `CodeFacts` 演示概念，不代表当前 owner/span 投影后的实际路由结果。
> 当前系统必须分别计算 exact Tag 与 file-hint Tag，只有 exact Tag 能绑定专项 RQ 和正式
> retrieval Dimension。
...
## 10. HostSummary 不是 Parser 原始字段（兼容字段说明）

> `HostSummary` 仍是 ReviewUnit 的兼容背景字段，但不再承担精确事实和 Retrieval 主输入。
> 当前精确事实来自 `UnitFactScope.unit_exact`；文件级集合只能作为 `file_hints`。下文保留旧
> 模型是为了帮助读取历史 JSON，不能据此设计新调用方。
...
## 11. 不要混淆的下游数据模型（历史模型，禁止新代码依赖）

> 下文 `CodeFeatures/RetrievalUnit/MrContext` 是早期兼容模型说明，已经被正式
> `UnitFeatureProfile`、`ContextPlanResult` 和 `RetrievalUnitRequest` 链路取代。新代码不得
> 按下文类形状实现；当前合同见
> [Feature Routing](../modules/04-feature-routing.md) 和
> [Retrieval](../modules/06-retrieval.md)。
...
早期 `CodeFeatures` 没有包含 `attributes`；当前正式模型不再使用该类作为主合同。
...
早期 `MrContext` 只保存所有 Unit Tags 的维度并集；当前正式实现已经保存 Unit 级
`retrieval_dimensions/routing_dimensions`，不再采用这里的建议模型。
