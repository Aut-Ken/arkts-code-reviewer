...
当前实现已经完成 RU-0～RU-3：独立 ReviewUnit Golden、collision-safe identity、多 owner、
质量传播，以及基于 Parser v2 occurrence 的 parse-once/Unit fact scope。它仍不是完整上下文
规划器；精确 ChangeSet 和有界关联上下文分别留给 RU-4/RU-5。Parser v1、FileAnalysis 和
ReviewUnit Golden 继续保持独立。
...
RU-3 已为 file result 增加 head `source_ref_id` 并输出 `review-unit-build-v2`；RU-4 再为
顶层结果增加 `change_set_id` 和
`unassigned_change_atom_ids`。RU-2 不提前发明匿名 revision 或精确 diff 身份。

RU-3 已在 FileAnalysis 中提供精确 `field_region/import_region`，但当前 `FileHunk` 选择算法
仍以 declaration 为 owner，region 行保持 fallback/unsupported；RU-4 会结合精确 ChangeAtom
完成 region owner 映射。ReviewUnit 不新增正则 RegionResolver 假装精确 owner。
...
RU-4        待实现：ChangeSet/base/head target
