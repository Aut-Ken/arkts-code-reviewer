...
Loader 和 `FeatureRouter(config=...)` 还支持显式 `tag-config-v2/v3`。v1/v2 schema
语义已冻结：v2 只增加 `any_import_use`；v3 再增加 `any_symbol_leaf`，并仅用于
FR-02 candidate。仓库默认配置、wheel defaults 和 `CodeAnalyzer` 仍运行
...
- `feature-routing-v1` signal 严格只有 `kind/value`；`feature-routing-v2` 的归一化 signal
  可原子增加 `operator/normalized_value`，当前 operator 只能是 `any_symbol_leaf`。
...
`any_symbol_leaf`。`tag-config-v3` 的 `any_symbol_leaf` 当前只由 FR-02 影子工具
显式注入。两者都不会改变默认 24 个 Tag、Dimension、Review Question 或 v1
Golden；后续每个规则都必须经过独立 shadow truth 和真实 P/R 门禁后，才能迁移
正式配置。
...
schema version 和 Tag config 内容版本不是同一概念。支持 `tag-config-v3` 不表示默认
配置已升级为 `tags-v3`，也不表示 candidate 已激活。
...
规范化值，v3 fingerprint 同时包含 `any_import_use` 与 `any_symbol_leaf`。
...
改写 expected 或 v1 signal shape。FR-02 的 `tag-config-v3 + feature-routing-v2` 由独立
Parser → ReviewUnit → FeatureRouter E2E 和 candidate report 验证；其 contract gate 只是
候选合同重放门禁，不是生产质量或激活门禁。
...
正式 Feature Routing v1 已完成；`feature-routing-v2` 目前只是 candidate provenance
schema，不是生产升级。当前仍没有实现以下下游或扩展能力：
...
- `any_symbol_leaf` 只比较最后一个点分段，无法区分 ArkUI component/page 与普通
  class 的同名方法；这仍是 FR-02 的生产激活 blocker。
