...
截至 `a83eeb6`，默认 `tags-v1` 仍是下面 24 个 Active Tag；近期 relational-store/lifecycle
工作改进了触发器、影子合同和评测治理，但没有新增 Active Tag。真实 taxonomy 扩展必须从代码
语料发现候选，再逐个经过 Truth 与 shadow 门禁，不能把“支持了新 matcher operator”写成
“已经增加新 Tag”。
...
48-probe 只计算项目 reference 就约 727M pair NFC chars。D1b-1 实现前冻结的
`d16b5f9d9bbac7040af9d315e52a98c016197d33` 只读快照是
563 个 regular blob、561 个 UTF-8
document、560 个 unique blob、558 份 unique text、56 个 tokenizer issue；同口径 48-probe lower
bound 为 743,761,104 pair NFC chars。这些都是 pinned 历史快照，不是滚动 “current HEAD”
指标；正式 campaign 必须对实际 candidate commit 重新计算 inventory，并继续对 oversize、
tokenizer 和 work budget 保守 abstain。
...
self-hash 不够。这里的 full rebuild 从调用方提供并通过 self-hash 校验的 PairSelection 根开始，
不验证 Pair source Git provenance 或声明 closure 的真实 blob bytes。Custodian 不得担任 Pair
reviewer；approver 不得是 reviewer/custodian。审批证明和记录时间不得早于 release，当前秒级
timestamp 合同允许相等；failed/not-eligible report 不能支持 approved。
...
行为和 fingerprint 均不改变。即使 ApprovalReceipt 的 decision 为 approved，其 scope 也只允许
未来 verified screening policy semantics，不批准当前 v1 policy、不激活 screening，也不使 Tag
Evidence qualified。

Stage 2D1b-1 与 Stage 2D2a 是 Stage 2D1 后的并行分支；D2a 不消费 D1b-1。Stage 2D2a
增加独立的 consensus publication bridge，并完整重建 Stage 2C/2D1
...
当前优先执行 `TAG-CORPUS-01`：对 pinned `applications_app_samples` 做只读事实统计，先回答
现有 24 个 Active Tag 覆盖了什么、漏掉了什么，再形成带真实样本、hard negative、trigger、
Dimension/RQ 和 Parser/owner 风险的候选清单。该阶段不自动修改配置；每个候选单独经过人工
Truth、shadow 和量化门禁后，才决定 Reject、Draft 或 Active。

之后仍应分别建立 relation discovery、Knowledge/Retrieval、Rules 和 Prompt/Final Review
Golden。所有下游和候选迁移都不能反向改变 `unit_exact/file_hints`、Tag activation trace、
retrieval policy 或 QuestionBinding truth。
