...
因此本模块状态仍是 `partial`：来源、标准化、候选提取、确定性标注、双审 consensus 和
publication 代码合同已经实现，但真实人工策展与首个发布数据集尚未完成。
...
| YAML | ruamel.yaml |
| DB | PostgreSQL + psycopg3（由 Retrieval runtime 持久化 publication index） |
