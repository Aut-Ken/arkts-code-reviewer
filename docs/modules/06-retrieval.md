...
`AITagResponseValidation` 不是 Builder 所需的 trusted Hybrid 输入，不能直接喂给 V2。
...
尚未实现 V2 `ai_inferred/text_keyword` MatchScope、各来源独立 candidate pool、AI 权重、
RRF shadow ranking 或 V2 EvidencePack 组装。因此 V2 schema/Builder 的存在不表示 Phase C
Retrieval shadow 已可执行。
