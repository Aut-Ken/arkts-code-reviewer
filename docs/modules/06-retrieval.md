...
- PostgreSQL 17 + pgvector 0.8.5 的迁移、不可变索引发布、完整 round-trip 校验和原子 alias
  ...
  `jinaai/jina-embeddings-v2-base-code`，768 维、CPU 本地运行。
...
模型 ID、维度和 `--local-files-only` 也可通过 CLI 显式传入。配置 loader 拒绝重复 YAML key、
未知字段、越界值、重复 authority 和破坏精确匹配优先级的权重。
...
| Embedding | FastEmbed 0.8，Jina code 768D，CPU |
