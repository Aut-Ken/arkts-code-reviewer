...
API/组件/Tag/全文       scenario embedding
        |                    |
        +---------+----------+
                  |
                  v
...
```text
matched_by / match_reason / score / rank_detail / source_anchor
```
...
```text
1. 50~100 条人工确认 Clause
2. Pydantic Query/Evidence 契约
3. API/组件/Tag 精确检索
4. Golden Set 和指标
5. scenario + Embedding
6. RRF 融合
7. 数据证明需要时再上 Reranker
8. 缓存、增量索引和服务化
```
