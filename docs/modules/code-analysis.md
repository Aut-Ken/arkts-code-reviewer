...
输出一：检索模块输入（**照 retrieval.md §1.3 履约**——Unit 批量请求，
`triggered_dimensions` / `token_budget` MR 级共享，`code_features` /
`intent_summary` Unit 粒度）：

```jsonc
{
  "mr_context": {
    "triggered_dimensions": ["DIM-01", "DIM-06"],
    "token_budget": 8000   // 透传自编排配置，本模块不决策其值
  },
  "units": [
    {
      "unit_ref": "PhotoWall.loadImages@src/pages/PhotoWall.ets",  // 命名约定见 §6.2
      "code_features": {
        "components": ["Image", "Grid"],
        "decorators": ["@State"],
        "apis": ["setInterval", "image.createPixelMap"],   // canonical 形式
...
批量规模：典型 MR 约 1~20 个 Unit，检索批量接口上限 50，超大 MR 由编排层分批
（分批归编排层职责，本模块单次输出不设限）。
...
### 3.4 白名单同源（✅ 已与检索模块对齐确认）

retrieval.md §3.1 中条款解析器的 `raw_keywords` 提取依赖 SDK 白名单。
**两模块共用本管道的同一份产物**——

- 查询侧（本模块）与索引侧（检索条款解析器）使用同一词汇表与同一命名规范化规则，
  否则关键词召回会因词形不一致（如 `PixelMap` vs `image.createPixelMap`）失效；
- 归一约定：索引侧按 bare + 别名表归一为 canonical 存储，查询侧按 import 解析
  天然发出 canonical；**bare → canonical 是一对多映射**（如 `getContext` 在多个
  模块均存在），别名表支持歧义映射，索引侧遇歧义裸名时将所有候选 canonical
  全部存入（召回优先）——此约定写入产物 meta，所有消费方共同遵守；
- 检索侧每条 chunk 记录归一所用 `whitelist_version`；
- 管道与别名表归属本模块，检索模块以数据依赖方式消费（产物文件 + 版本号）。
...
```jsonc
{
  ...
  "unit_ref": "PhotoWall.loadImages@src/pages/PhotoWall.ets",  // 检索契约中的 Unit 唯一引用
...
`unit_ref` 命名约定（与 retrieval.md §1.3 对齐）：
...
| L0 解析降级（三级兜底，无符号边界） | `hunk-L{start}-L{end}@file`（与 `context_degraded` 标记联动） | `hunk-L40-L58@src/pages/PhotoWall.ets` |

一个 MR = 每文件若干 Review Unit；检索 query 与评审 Prompt 均以 Unit 为消费单位
（检索契约按 Unit 批量请求，clauses 按 unit_ref 分组回挂，见 §1.3）。
...
| # | 事项 | 状态 |
|---|---|---|
| 1 | 输出契约（Unit 批量请求：unit_ref + code_features + intent_summary，MR 级共享 triggered_dimensions / token_budget） | ✅ 照 retrieval.md §1.3 履约，双方已确认 |
| 2 | 白名单同源（§3.4）：共用产物 + bare→canonical 归一（歧义全存）+ whitelist_version，别名表归本模块维护 | ✅ 检索模块已确认 |
| 3 | triggered_dimensions 计算归属本模块 | ✅ 契约已明示，登记备查 |
| 4 | tags.yaml 加入配置治理家族（与 dimensions.yaml / 路由表同套），CI 交叉校验 | ✅ 本模块单方面实现 |
| 5 | unit_ref 命名约定（标准 / 整文件 / L0 降级三态，见 §6.2） | ✅ 双方文档同表 |
| 6 | token_budget 透传自编排配置，两模块均不决策其值 | ✅ 双方文档同标注 |
| 7 | 检索缓存键 =（Unit 特征包 hash, index_version）；intent_summary 为 llm provenance 时非确定性、略降命中率，已知可接受 | ✅ 已确认 |
...
模块形态：**纯 Python 库**（`src/` 布局 package），不含服务框架，与检索模块
同形态；由接入层服务 import，在评审管线中作为检索的上游同进程调用。
白名单管道（§3）是独立可执行工具（`tools/build_sdk_whitelist.py`），
与库共享解析基建。

与检索模块（retrieval.md §9）的技术栈基线**完全对齐**：语言版本、包管理、
数据契约、YAML、LLM 调用、测试框架、质量工具全部同套；差异仅在各自职责
特有的依赖（本模块加 tree-sitter，不需要 DB 一侧）。

| 层 | 选型 | 理由 / 与检索模块的对齐说明 |
|---|---|---|
| 语言 | Python 3.12（`>=3.12,<3.13`） | 与 retrieval.md §9 同版本约束，同仓协作零摩擦 |
| 包管理 | uv + `pyproject.toml` | 同检索模块；语法库等二进制 wheel 一并锁定于 `uv.lock` |
| 数据契约 | Pydantic v2 | `CodeFeatures` / `ReviewUnit` / `Tag` / 白名单产物等模型；**§1.3 输出契约（`mr_context` + `units[]`）与检索模块输入共用同一份模型定义**，单一来源，从根上杜绝两侧契约漂移 |
| L0 词法层 | 标准库 `re`（自研词法规则集） | 零第三方依赖的保底路径（§2），任何环境可运行 |
| L1 结构层 | tree-sitter + tree-sitter-typescript（Python 绑定） | 容错解析 ArkTS（`struct→class` 预处理，§2）；本模块特有依赖，纯 Python 生态内、无 Node.js；语法库升级必须过解析器 golden set 回归（§8）后才允许合入 |
| 白名单管道解析 | 复用同一 tree-sitter-typescript + JSDoc 注释正则提取 | `.d.ts` 为标准 TypeScript，零方言障碍（§3.1）；管道与运行时解析器共享解析基建，不另起技术路线 |
| YAML | ruamel.yaml | 读 `tags.yaml` / `dimensions.yaml`，写白名单产物；与检索模块统一选型，保留注释与键序，产物 git diff 友好（§3.3） |
| LLM 调用 | 自研 LLM Gateway 接口（OpenAI 兼容 SDK → DeepSeek） | 与检索模块 LLM 增强共用**同一个 Gateway 抽象**（architecture.md）；L2 语义标签 + intent_summary 合并为一次调用（§7） |
| 测试 | pytest | 解析器 golden set 以文件 fixture 回归（真实 .ets → 期望特征 JSON）；本模块无 DB，**不需要 testcontainers**——与检索模块的合理差异，非不一致 |
| 质量 | ruff（lint + format）+ mypy | 同检索模块单工具链 |
| 配置 | pydantic-settings | 白名单产物路径、SDK 版本、LLM Gateway 端点等走环境变量，不硬编码机器相关路径（architecture.md §1.1） |

**明确暂不引入**（引入前需重新评审本节）：

- **Node.js 运行时 / ohos-typescript sidecar**：官方解析器保真度最高，但引入
  跨语言进程管理成本；作为 L1 精度不达标时的升级路径（触发条件见 §2 与 §10
  待定项），届时专项评审。
- **完整 TypeScript 编译器方案**（tsc / babel / SWC 等）：本模块只需特征提取，
  不需要类型检查与代码生成，重量与需求不对称。
- **LangChain / LlamaIndex**：与检索模块同口径——L2 是一次纯 LLM 调用，
  编排框架无用武之地。

包组织方式（与检索模块同 package 的子模块 vs uv workspace 独立 package）
属实现期决策，两模块开工前统一定，不影响上表任何选型。
