...
status: proposal
implementation: dispatch_contract_slice_implemented
...
截至 2026-07-18，`src/arkts_code_reviewer/hybrid_analysis/` 已实现四批能力：第一批是第
...
验证。真实 DeepSeek transport/API key 接入、provider outer response adapter、运行预算、合规批准、
真实重试与 Retry-After 处理、RetrievalRequestV2、Retriever 接线、真实模型评测和生产启用仍未
...
render-only 能力，没有形成真实网络调用路径；未来真实发送必须另建 authorized version。
...
分支。真实发送必须另建带 trusted dispatch-attempt receipt、预算和合规门禁的 authorized
schema/version。

DeepSeek 官方已确认 OpenAI-compatible Chat Completions，但真实 provider outer response 解析、
HTTP transport、API key、timeout/Retry-After/重试、预算和合规门禁仍未实现。未来 adapter
必须只消费验证过的 envelope，并对空 content、`finish_reason=length`、schema-invalid 和
provider 变更 fail closed。只有未来 trusted dispatch-attempt receipt 和 provider outer-response receipt 同时
闭合请求、实际尝试和供应方外层响应后，才能定义向正式 `AITagAnalysisResult`/
`AITagExecutionOutcome` 的转换。这些未实现能力不能从当前 render-only 合同中推断。
...
`HybridFeatureAnalysisResult` 消费。未来只能在 trusted dispatch-attempt receipt 与 provider outer-response
receipt 都实现后，通过新的显式 formalization contract 产生正式 Result/Outcome；不能原地提升
当前 validation artifact。
...
`AITagAnalysisResult` schema 只用于表示未来经 trusted dispatch-attempt receipt 与 provider
outer-response receipt 共同证明、并通过 closed validation 的 formal provider 结果。当前
...
不能用空 judgments 冒充 valid response。上述 formal producer 和收据链尚未实现；当前只有 schema、seal
函数和调用方提供 artifact 时的图一致性 verifier。
...
这个本地处理器接收的 raw completion 仍是 transport-neutral 对象；从 DeepSeek HTTP 外层响应中
提取 content、model、finish reason、system fingerprint 和 usage 的 provider outer adapter 尚未实现。
...
| Formal Result/Outcome producer | 未实现；需要 trusted dispatch-attempt receipt 和 provider outer-response receipt |
| 真实 DeepSeek 客户端 | 未实现；未来使用独立 provider outer adapter 和 HTTP transport 调用官方 OpenAI-compatible endpoint |
| 并发与重试 | 未实现；未来使用 `asyncio` + 有界 semaphore，并为 timeout/429/5xx 实现有界重试 |
...
| Secret 管理 | 未实现 API key 接入；未来只从部署 secret provider 读取，不写入配置、artifact 或日志 |

真实 transport 落地时建议新增 optional dependency group，例如 `ai-analysis`，避免没有模型需求的本地解析和 Golden
被远程客户端依赖污染。当前 render/validation 合同不依赖 HTTP 客户端；`httpx` 只由 embedding
相关可选依赖组引入，并不是基础依赖；
...
| DeepSeek API 不可用 | 当前只能将未验证 failure claim 记为 validation `unavailable_claim`；真实 failure 与 formal Outcome 待 provider/attempt receipts 实现 |
| DeepSeek timeout/429 | 当前只校验 scripted/unverified failure claim；未来 authorized adapter 实现有界重试和 formal 降级 Outcome |
...
#### Dispatch/响应合同与未来 DeepSeek Adapter
...
真实 provider adapter 落地后还必须另行覆盖：

- 200 外层响应的严格字段提取；
- trusted dispatch-attempt 与 provider outer-response receipts 的内容寻址、交叉引用和 formal Result/Outcome 转换；
- 429/5xx/timeout 重试、non-retryable 4xx fail-fast 和 Retry-After；
- API key 不出现在 artifact/log；
- 如启用 raw response quarantine，其受控存储、路径、权限与保留期。
...
- 真实 DeepSeek V4 Pro provider outer adapter 和 HTTP transport；
- trusted dispatch-attempt receipt、provider outer-response receipt 和 formal Result/Outcome producer；
- API key/secret provider、输入输出/MR 预算、并发、timeout/Retry-After/有界重试和账本；
...
| Formal Result/Outcome producer | 未实现 | 需要 trusted dispatch-attempt + provider outer-response receipts，不能提升 validation artifact |
| DeepSeek V4 Pro 真实 Tag 判断 | 未实现 | 仍需 transport/API key/budget/compliance/retry 与真实 shadow |
