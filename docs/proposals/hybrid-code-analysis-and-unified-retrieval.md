...
status: proposal
implementation: repository_r3_valid_shape_smoke_and_multi_unit_shadow_evaluation_contract_implemented
...
截至 2026-07-19，`src/arkts_code_reviewer/hybrid_analysis/` 已实现八批能力：第一批是第
...
这些 Golden 只证明确定性合同；新增 shadow evaluation Golden 也不证明 DeepSeek 判断正确，不提供
Tag Truth，不能证明 DeepSeek 或真实文档质量。
...
- multi-Unit judgment distribution 的 non-formal 记录合同已实现，不参与检索；尚未运行真实代码 campaign；
