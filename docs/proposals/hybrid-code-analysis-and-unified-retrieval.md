...
status: proposal
implementation: repository_r3_smoke_campaign_execution_formal_v2_attestation_and_retrieval_v3_trusted_builder_contracts_implemented
...
截至 2026-07-20，`src/arkts_code_reviewer/hybrid_analysis/` 及 Retrieval 的 hybrid 接口已实现十三批能力：第一批是第
...
该批只完成 schema/loader/Builder 的结构边界；`RetrievalService`、MatchScope、各来源独立
candidate pool、AI 权重、RRF shadow ranking 和 V2 EvidencePack 执行都未实现。现有 Campaign
summary/ResponseValidation 是 non-formal 产物，不能直接作为该 Builder 要求的 Hybrid 输入。
...
Dimension coverage。普通 self-hashed V3 JSON 不是执行 authority。当前 `RetrievalService` 仍只接受
v1，V3 没有 candidate pool/权重/RRF shadow/EvidencePack runtime，因此没有改变任何实际检索结果。
...
真实代码 DeepSeek campaign、V2/V3 Retriever 接线/影子排名、真实模型质量评测或生产启用。环境 credential provider 只实现了
...
RetrievalRequest 合同，其中 V3 是 Formal V2 AI signal 的受信 construction gate，当前可执行 runtime
仍只有 v1。
...
当前 4～7 项的 schema、attempted evidence formalizer/verifier 和受信 request-construction gate 已实现；
但没有部署 key/真实 formal live，V3 也没有进入 `RetrievalService`、ranking 或 EvidencePack runtime。
...
`RetrievalService` 执行。live Campaign safe summary 或 `AITagResponseValidation` 不是完整 Formal V2
evidence，不能直接转成 V3 输入；历史 summary 也不能事后追认。
...
第 8.6 节的 Retrieval V2 与后述 V3 都只有 request-construction 合同，当前 Retrieval runtime 仍是 v1。
...
V2 schema/loader/Builder 中的这些字段已实现，但用于候选匹配的
`ai_inferred/text_keyword` `MatchScope` 尚未实现。因此不能把 schema 字段存在描述为 v1 或
V2 Retrieval runtime 已实现。
...
flag 绕过 verifier。当前 `RetrievalService` 明确拒绝 V3；尚无 V3 MatchScope、candidate pool、AI
权重、RRF shadow ranking 或 EvidencePack runtime。
...
| Retrieval V3 trusted construction gate | 已实现独立 closed schema、strict loader、code-first renderer、逐 Unit Formal V2 evidence exact coverage、完整 v1 baseline 复核和不可序列化 verified wrapper；当前 `RetrievalService` 仍只接受 v1，无 V3 candidate pools/ranking/EvidencePack runtime |
...
它不进入 Hybrid 或 Retrieval。另有一条已实现到 V3 construction gate 的合成合同链，但不是生产能力：
...
`RetrievalService` 仍拒绝 V3；`VerifiedRetrievalRequestV3 -> fixture KnowledgeIndex -> EvidencePack` 是未实现
的 Retrieval shadow runtime，不能从 request-construction tests 推断其结果。
...
- `retrieval-request-v2` schema/loader/structural Builder 与 `retrieval-request-v3` formal evidence gate/
  verified wrapper 已实现，但二者仍不可由当前 `RetrievalService` 执行；
- AI positive 使用独立 `ai_inferred` scope 参与影子 structured candidate；
- AI not_supported 不过滤任何正式候选；
- 与 static+vector 结果并排；
- 不改变用户可见 Evidence。

除 request construction 合同外，本 Phase 的 `ai_inferred/text_keyword` MatchScope、独立 candidate
pool、AI 权重、RRF shadow ranking 与 V2/V3 EvidencePack 执行均未实现。
...
| RetrievalRequestV2 | 独立 closed schema、strict loader、code-first vector renderer 和 deterministic structural Builder 已实现 | 作为未来 V2 runtime 的唯一请求合同，不新增平行 Intent 真值；当前不接 RetrievalService，无 MatchScope/pool/AI 权重/RRF shadow/EvidencePack 执行 |
| RetrievalRequestV3 | 独立 closed schema、strict loader、Formal V2 complete-evidence verifier gate、code-first renderer 和 non-serializable verified wrapper 已实现 | 只有 verified positive 可投影；serialized JSON 不是 authority；当前不接 RetrievalService，无 V3 candidate/ranking/EvidencePack runtime |
