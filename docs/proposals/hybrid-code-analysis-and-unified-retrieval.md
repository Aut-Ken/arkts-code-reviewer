...
status: proposal
implementation: repository_synthetic_usage_extension_compatibility_implemented
...
截至 2026-07-19，`src/arkts_code_reviewer/hybrid_analysis/` 已实现七批能力：第一批是第
...
该 smoke harness 的自动化执行测试全部使用 injected transport，不发起网络请求。仓库在两次用户逐次
明确批准固定仓库 Prompt、24-Tag taxonomy 与内置合成代码外发后，执行过两次真实 DeepSeek smoke：
...
两次原始响应均按既定策略没有落盘；第二次 diagnostic 只能定位结构层级，不能恢复具体未知字段名或
值。HTTP status、latency、size 与 reason 来自当次终端脱敏 summary；仓库只保留忽略于 Git 的本地
consumption marker，没有可供第三方离线重建运行观察的已提交 artifact。marker 只证明对应本地
reservation/Plan 已被消费，不证明 HTTP status、latency 或 response；两次授权均已消费且没有重试。

r3 实现后的默认 inspect-only 重建结果为新 Plan
`ai-tag-shadow-plan:sha256:20d55d4ac2310f267dd972a5dad9025041e78408a40be0e8d2aa7e7d96d936d9`；
wire body 仍是 25839 bytes，hash 仍为
`sha256:9165e9853d1a907546a6c6c786de849c1d8b7cc041202f172ba83d43d5f622d0`。这次 inspect 明确
`network_attempted=false`，没有读取 API key 或调用 DeepSeek；它只证明 r3 policy 改变了 Plan identity
而未改变本次请求正文。旧 r2 Plan 的批准不能复用于该新 Plan，当前尚无 r3 live 结果。
...
惰性读取 `DEEPSEEK_API_KEY` 的能力；上述两次本地 attempt 不是部署批准或正式运行记录。合同、
...
approval verifier 或真实预算 ledger。2026-07-18 与 2026-07-19 的两次 live connectivity smoke 都收到
HTTP 200，但均 outer contract invalid；它们都没有产生合法 response receipt 或 judgment。首次发生在
结构化 diagnostic 之前；第二次在 r2 provider snapshot 下由 parser v1/diagnostic v1 将漂移定位到
`usage` 直接未知字段，但 raw response 已丢弃，不能再恢复字段名和值。
...
项目代码调用配置。仓库只执行过两次 8.3.4 固定合成资产的 live connectivity smoke；两次均收到 HTTP
...
可以相同，仍必须以最新 inspect 实测为准。两次旧授权都已消费，任何后续 live 都必须另获一次明确
...
| DeepSeek outer adapter/transport | 已实现 r3/parser-v2 严格外层 parser、diagnostic v2 与模块私有 `httpx` transport；仅受控丢弃最多 16 个直接 `usage` 扩展，其余语义/known usage 仍严格；固定 endpoint/TLS、禁 redirect/env proxy、绝对 deadline；此前两次 live 请求均收到 HTTP 200 后 outer-invalid |
...
| Repository-synthetic smoke harness | 已实现 package-contained hash-locked 代码单样例、仓库 Prompt/Catalog identity、默认 inspect-only CLI、显式 exact Plan/body controls、同一 state-dir 的本地 marker 与脱敏 summary v4；仅完整 valid shape 投影 canonical 24 项 `tag_id+decision`，失败为空且不落 marker；自动化测试零网络；两次 live 收到 HTTP 200 但无合法 parsed response/judgment，资产 public/release approval 未证明 |
...
- 当前 `DeepSeekShadowRunner` 的 real-provider 路径统一经 `dispatch_once` 消费 capability，以及
  ...
  exact-body approval、同一 state-dir 的原子 attempt marker 和脱敏 summary（已实现；两次 live 收到
...
- DeepSeek V4 Pro provider outer adapter、私有 HTTP transport 与一次性 shadow observation（代码已实现；
  已有两次 HTTP 200/outer-invalid 本地 connectivity attempt，无合法 response judgment）；
...
- 固定仓库 Prompt/taxonomy 与合成代码的 inspect-first smoke harness（代码与 synthetic transport 测试已实现；
  两次真实调用收到 HTTP 200 但未通过各自 frozen outer contract）；
...
| Provider transport/receipts | 已实现 r3/parser-v2、ObservedResponse receipt v2、synthetic/httpx 合同测试、隐私安全 outer diagnostic v2 及 inspect-first 单样例 smoke harness | 两次真实 API 请求均 HTTP 200/outer-invalid；r3 兼容代码尚未 live 验证；所有 observation 非 formal、非 Tag Truth |
