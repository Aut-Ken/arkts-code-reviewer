...
status: proposal
implementation: repository_r3_smoke_shadow_evaluation_campaign_execution_controlled_synthetic_campaign_smoke_and_retrieval_request_v2_structural_builder_implemented
...
截至 2026-07-20，`src/arkts_code_reviewer/hybrid_analysis/` 及 Retrieval 的 hybrid 接口已实现十一批能力：第一批是第
...
V2 Retriever 接线/影子排名、真实模型质量评测或生产启用。环境 credential provider 只实现了
...
`HybridFeatureAnalysisResult` 是上游审计产物，`RetrievalRequestV2` 是未来 V2 runtime 的唯一请求
合同；不再创建平行 `UnifiedRetrievalIntent` artifact。当前已实现 V2 的独立 schema、strict
loader 和 deterministic Builder，但它还不是可由 `RetrievalService` 执行的请求。Builder 先重建 v1
baseline 与 Analysis Card，再验证调用方提供的 ModelView/Request/Outcome/Result/Hybrid 引用图。
live Campaign safe summary 或
`AITagResponseValidation` 既不是 formal Result/Outcome，也不能直接转成 V2 输入。
...
- multi-Unit selection/Plan manifest、inspect-only projection、judgment distribution 与完整非 formal
  ...
  人工 Truth/P/R 或 provider/runner signature；
- 测量 full-24 single/batch、token、invalid、latency、位置效应和 Unit-exact disagreement。

### Phase C：Retrieval shadow

- `retrieval-request-v2` schema/loader/structural Builder 已实现，但仍不可由当前
  `RetrievalService` 执行；
...
除第一条的结构合同外，本 Phase 的 `ai_inferred/text_keyword` MatchScope、独立 candidate
pool、AI 权重、RRF shadow ranking 与 V2 EvidencePack 执行均未实现。
