...
status: proposal
implementation: repository_synthetic_outer_diagnostics_and_safe_decision_summary_implemented
...
自由文本的 summary。summary v3 只在 outer 与 inner 全部通过时额外投影完整 canonical 24 项
...
该 smoke harness 的自动化执行测试全部使用 injected transport，不发起网络请求。2026-07-18 在用户
明确批准固定仓库 Prompt、24-Tag taxonomy 与内置合成代码外发后，仓库执行过**一次**真实
DeepSeek smoke：固定 `httpx` transport 观察到 HTTP 200，latency 为 7686 ms，但响应没有通过当时
冻结的 provider outer schema，最终为 `invalid_output/provider_outer_contract_invalid`，没有生成
ObservedResponse receipt、inner validation、usage 或任何 Tag judgment。原始响应按既定策略没有落盘
或进入 summary，因此该历史 attempt 不能反推具体漂移字段，也不能证明模型 Tag 能力。HTTP status、
latency 与 reason 来自当次终端脱敏 summary；仓库只保留忽略于 Git 的本地 consumption marker，
没有可供第三方离线重建该运行观察的已提交 artifact。
该 marker 只证明对应本地 reservation/Plan 已被消费，不证明 HTTP status、latency 或 response；这些
运行事实不能从 marker 重建。

仓库仍没有部署侧外发审批 verifier、真实预算 ledger/reservation、生产 secret manager、通用并发与重试、
KMS/运行器签名、ResultV2/OutcomeV2 formalization、可接受的真实 DeepSeek response/judgment、
RetrievalRequestV2、Retriever 接线、真实模型评测或生产启用。环境 credential provider 只实现了
惰性读取 `DEEPSEEK_API_KEY` 的能力；上述一次本地 attempt 不是部署批准或正式运行记录。合同、
...
非 formal 的 shadow adapter 与固定合成样例 harness，但仍没有 production/formal client、部署审批
或正式 Result 转换。所有 provider、model、`system_fingerprint`、thinking、
...
approval verifier 或真实预算 ledger。2026-07-18 的首次 live connectivity smoke 收到 HTTP 200，
但 outer contract invalid；它没有产生合法 response receipt 或 judgment。后续 parser 增加
`deepseek-outer-response-diagnostic-v1`，以内容寻址方式绑定 Plan、response-body hash/size 与 parser
contract，只记录 `utf8/json/top_level/schema` 阶段、封闭 error type 和脱敏字段路径，不记录值、
content、原始异常或原始响应。该能力不能追溯诊断已经丢弃 raw response 的历史 attempt。
...
结构化 diagnostic 分类；该接受语义变更已将 provider contract snapshot 升为
`deepseek-chat-completions-2026-07-18-r2`，没有原地改写旧 snapshot。历史 r1 policy/Plan 仅允许按其
self-identity 反序列化供审计；当前 outer parser 只接受声明 r2 snapshot 的 Plan，并在读取 body 前拒绝
r1。deterministic rebuild、Authorization Gate 和 dispatch 进一步只接受当前 builder 生成的 r2 Plan，
因此 r1 artifact 不能重新获得解析或执行资格。当前未签名 receipt 只能闭合本地观察，
...
项目代码调用配置。仓库只执行过一次 8.3.4 固定合成资产的 live connectivity smoke；它收到 HTTP
200 后被 outer contract 拒绝，没有产生 Tag judgment。8.3.4 的本地 smoke controls 只允许一种
...
live summary v3 只保留标准化 status/reason、usage、latency、decision counts、artifact IDs，以及
response body hash/size。只有 observation 与 response validation 都是 `valid_shape`、彼此引用一致，
...
即使未来一次真实仓库资产/合成代码调用返回 `valid_shape`，也只证明 endpoint、鉴权、JSON 合同和本地
receipt 链在该次尝试中工作，并产生一份未签名的固定合成 Unit 模型观察；不证明 Tag Precision/Recall、
真实项目外发合规或生产可用性。

首次 live attempt 发生在上述 diagnostic v1 与 summary v2/v3 实现之前：固定 transport 收到 HTTP 200，
但旧 summary 只能报告 `provider_outer_contract_invalid`，且 raw response 随进程退出丢弃。因此新增
diagnostic 不得被描述为已经确定该历史失败原因；任何后续 live 都必须另获一次明确授权和独立 attempt
identity，不能删除 marker 或更换 state directory 来伪装成同一次批准下的重试。
...
provider outer 失败必须生成内容寻址且可由同一 raw bytes 重建的隐私安全 diagnostic；diagnostic 只
...
一致，因此缺失值显式降级为 `not_reported`/null，不得虚构零值；未知字段仍保持 fail closed。
...
| DeepSeek outer adapter/transport | 已实现严格外层 parser、隐私安全内容寻址 diagnostic 与模块私有 `httpx` transport；固定 endpoint/TLS、禁 redirect/env proxy、绝对 deadline；一次 live 请求收到 HTTP 200 后 outer-invalid |
...
| Repository-synthetic smoke harness | 已实现 package-contained hash-locked 代码单样例、仓库 Prompt/Catalog identity、默认 inspect-only CLI、显式 exact Plan/body controls、同一 state-dir 的本地 marker 与脱敏 summary v3；仅完整 valid shape 投影 canonical 24 项 `tag_id+decision`，失败为空且不落 marker；自动化测试零网络；一次 live 收到 HTTP 200 但无合法 parsed response/judgment，资产 public/release approval 未证明 |
...
- 200 外层响应、model/choice/role/tool/reasoning/logprobs/usage drift 的严格解析；
...
- summary v3 的 `validated_tag_decisions` 只允许从完整 valid response validation 投影 canonical 24 项，
  每项只有 `tag_id/decision`；inner/outer/provider failure 必须为空，三种 decision、顺序、counts 一致性、
  reason/evidence 不泄漏和 marker 不持久化必须由测试覆盖；
...
- 当前 `DeepSeekShadowRunner` 的 real-provider 路径统一经 `dispatch_once` 消费 capability，以及
  ...
  exact-body approval、同一 state-dir 的原子 attempt marker 和脱敏 summary（已实现；一次 live 收到
  HTTP 200 但 outer-invalid，不能处理任意项目代码，也不是资产发布证明、部署 approval 或真实预算）；
- summary v3 的完整 valid-shape `tag_id+decision` 脱敏投影（已实现；仅终端观察，不是新
  Result/Outcome artifact，不进入 Hybrid/Retrieval）；
- outer response 内容寻址隐私安全 diagnostic（已实现；绑定 Plan/body identity，只保存封闭错误分类和
...
- DeepSeek V4 Pro provider outer adapter、私有 HTTP transport 与一次性 shadow observation（代码已实现；
  已有一次 HTTP 200/outer-invalid 本地 connectivity attempt，无合法 response judgment）；
...
- 固定仓库 Prompt/taxonomy 与合成代码的 inspect-first smoke harness（代码与 synthetic transport 测试已实现；
  一次真实调用收到 HTTP 200 但未通过 frozen outer contract）；
...
| Provider transport/receipts | 已实现代码与 synthetic/httpx 合同测试、隐私安全 outer diagnostic 及 inspect-first 单样例 smoke harness | 一次真实 API 请求 HTTP 200/outer-invalid；所有 observation 非 formal、非 Tag Truth |
| Formal Result/Outcome producer | 未实现 | 现有 v1 结构 seal 无 receipt provenance；需要受信 attestation/trust registry 和带 refs 的 v2 |
| DeepSeek V4 Pro 真实 Tag 判断 | 未实现 | 有 HTTP 200 connectivity observation，但没有通过合同的 live judgment、真实模型 P/R、部署合规或预算证据 |
