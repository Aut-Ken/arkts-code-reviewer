...
status: proposal
implementation: repository_r3_smoke_shadow_evaluation_and_campaign_preparation_contracts_implemented
...
截至 2026-07-19，`src/arkts_code_reviewer/hybrid_analysis/` 已实现九批能力：第一批是第
...
| Multi-ReviewUnit campaign preparation | 已实现 evaluation-only manifest、从完整调用方 upstream graph 的 deterministic full rebuild、不含明文 Unit path/symbol 的 inspection projection，以及 caller-keyed Plan coverage + Envelope-bound validation adapter；不 dispatch、不读取 credential、不产生 sealed execution campaign，固定 not-qualified |
...
- 当前没有表示 `skipped_budget/not_run` 和所有 transport/outer failure 的完整 campaign-result schema；缺少
  ResponseValidation 时必须拒绝，不能静默省略或伪造成 `not_supported`。
...
- multi-ReviewUnit campaign manifest、从调用方 upstream graph 的完整重建、不含明文 Unit path/symbol
  ...
  inspect-only、零网络，不是 sealed execution
  campaign，既有 report v1 仍固定 not-qualified）；
...
- multi-Unit selection/Plan manifest、inspect-only projection 和 judgment distribution 的 non-formal 合同已
  实现，不参与检索；尚未实现完整 campaign execution-result，也未运行真实代码 campaign；
...
| Multi-ReviewUnit campaign preparation | 已实现 content-addressed manifest、不含明文 Unit path/symbol 的 inspection、完整 upstream rebuild 和 Envelope-bound validation-map adapter | 只冻结 selection 与 per-Unit outbound Plans；mapping 不绑定 Plan execution；不发送、不读取 credential、不证明 attempt/provider/Truth；不是 sealed campaign result |
