...
status: proposal
implementation: repository_r3_smoke_campaign_formal_v2_retrieval_v3_phase_c_and_phase_d0_evaluation_contract_implemented
...
截至 2026-07-20，`src/arkts_code_reviewer/hybrid_analysis/` 及 Retrieval 的 hybrid 接口已实现十五批能力：第一批是第
...
第十五批实现 Phase D0 离线文档评分合同。独立 `retrieval-document-truth-v1` 供调用方逐 Unit 提供唯一
`required/acceptable/irrelevant/forbidden` Clause 标签、critical Dimensions、Feature Config 与
development/calibration split；独立 `retrieval-shadow-evaluation-v1` 从 Truth、V3 request、Phase C
shadow result 和固定 Index 四个 self-hash roots 重建预算前/预算后双臂 Recall、coverage、Precision、
MRR、Knowledge gap、Dimension、token 与 hard-safety observations。报告同时保留全体 observational
aggregate 和每个非空 split 的独立 aggregate；paired quality delta 只消费 eligible Unit，安全违规不会
因 degraded 被隐藏。未标注 observed Clause、重复 source locator、root/config/budget/Index metadata 不一致
均 fail-closed。
report 固定 `serialized_audit_only`、not-qualified、非用户可见且不可进入 Prompt/Finding；当前 Truth 和
测试仍为 synthetic/caller-supplied，没有人工 consensus/Git seal、acceptance holdout、Shadow runtime
authority 重建、独立 rule-family leakage seal、生产知识或真实 P/R；split 是调用方标签，分别聚合不证明
不存在近重复泄漏。
...
V3 EvidencePack，也不会改变 v1 control EvidencePack。不可序列化 result wrapper 保留独立的完整
shadow/control construction snapshots，在现有 request/index/config/policy 重建之外，精确绑定 semantic
score、候选、diagnostics/degraded 和 control EvidencePack；公开访问不重新调用 embedding provider。
...
| Retrieval V3 trusted gate + Phase C shadow runtime | 已实现独立 closed schema、strict loader、code-first renderer、逐 Unit Formal V2 evidence exact coverage、完整 v1 baseline 复核、不可序列化 verified request/result wrapper、独立 shadow/control construction snapshots、五 pool 与 `static_vector/hybrid` 两 arm；标准 `RetrievalService` 仍只接受 v1，实验结果是 `retrieval-shadow-result-v1`，不是 EvidencePack |
...
- D0 schema/scorer/full-root rebuild 已实现：固定 Truth 标签、预算前/预算后双臂指标、Knowledge gap、
  eligible-only quality delta、all-observed hard-safety delta，以及互不混合的 development/calibration
  split aggregates；当前只通过 synthetic contract tests，artifact 固定 serialized-audit-only/not-qualified；
- 真实、独立复核并 sealed 的 development/calibration Truth、消融运行和候选选择尚未完成；
...
| Retrieval Document Truth / Phase D0 | caller-supplied Truth schema、strict loader、双臂 pre/post-budget scorer、development/calibration split aggregates、本地不变量和四 caller-root full rebuild 已实现 | self-hash 不证明人工 consensus/Git seal；report 为 serialized-audit-only，full rebuild 不恢复 Shadow runtime/policy authority；无 acceptance holdout、生产知识或真实 P/R |
