...
status: proposal
implementation: repository_r3_smoke_shadow_evaluation_campaign_execution_and_controlled_synthetic_campaign_smoke_implemented
...
截至 2026-07-19，`src/arkts_code_reviewer/hybrid_analysis/` 已实现十批能力：第一批是第
...
验证，没有真实 multi-Unit live campaign、人工 Truth/P/R、provider signature 或 trusted-runner signature；
新增的 CLI 也只接受 package-owned、hash-locked 的 4-ReviewUnit 合成 Campaign：默认 inspect-only，执行时
必须精确绑定 Campaign/Plan-set/caps/确认文本并逐 Plan 原子消费本地 marker。它不接受任意真实代码 Campaign，
且本轮没有运行真实 multi-Unit DeepSeek 请求。
...
KMS/运行器签名、ResultV2/OutcomeV2 formalization、真实代码 DeepSeek campaign、
RetrievalRequestV2、Retriever 接线、真实模型质量评测或生产启用。环境 credential provider 只实现了
...
| Multi-ReviewUnit campaign execution audit | 已实现 per-Plan runtime binding exact coverage、默认禁止真实与 injected transport、canonical sequential single-attempt/no-retry Harness、attempted/skipped-budget/not-run、local non-attempt receipt 与 inner/outer/provider failure sealed artifacts、机械 counts、persistent graph verifier 和可选 caller raw-bytes full rebuild；硬上限不可放大；固定 not-qualified，不进入 Hybrid/Retrieval/Finding；仅有合成/负向合同测试，未运行真实 multi-Unit live campaign |
| Repository-synthetic campaign smoke | 已实现 hash-locked 4-ReviewUnit 合成 Campaign、默认 inspect-only CLI、Campaign/Plan-set/caps/固定确认精确绑定、per-Plan `0700/0600` 原子 marker 与 metadata-safe summary；自动化测试只使用 injected transport | 只证明固定 synthetic 控制与执行连通合同；不接受任意真实代码 Campaign，本轮未发送真实 multi-Unit DeepSeek 请求，不提供 Truth/P/R/provider attribution |
...
- 固定 synthetic CLI 默认 inspect 时不得构造 credential、访问 `.env`/state 或 transport；execute 必须精确
  绑定 Campaign ID、Plan-set digest、全部 caps、固定确认文本和 per-Plan atomic marker，terminal summary
  不得输出 Unit path/symbol、代码、Prompt/body/raw response/reason/key/state path。该 CLI 不接受任意源码。
...
这些 Golden 只证明确定性合同；新增 shadow evaluation/campaign preparation Golden 也不证明 Plan
实际执行、DeepSeek 判断正确，不提供 Tag Truth，不能证明 DeepSeek 或真实文档质量。
...
它不进入 Hybrid 或 Retrieval。下面仍是后续目标链，不是当前已经实现的生产能力：
...
- package-owned hash-locked 4-ReviewUnit synthetic Campaign inspect/execute CLI、精确 Campaign/Plan-set/caps
  approval、per-Plan atomic marker 和脱敏 summary（已实现；自动化零网络，本轮未实际执行 multi-Unit live
  DeepSeek 请求，不接受任意真实代码 Campaign）；
...
- multi-Unit selection/Plan manifest、inspect-only projection、judgment distribution 与完整非 formal
  campaign execution-result 合同已实现，不参与检索；execution Harness 当前只有合成/负向测试，尚未运行
  真实 multi-Unit live campaign；固定 synthetic CLI 也尚未真实 dispatch，且没有人工 Truth/P/R 或
...
| Multi-ReviewUnit campaign execution audit | 已实现 canonical sequential per-Plan single-attempt/no-retry Harness、完整 disposition/outcome + local non-attempt receipt schema、sealed Unit/Result、aggregate rebuild、persistent graph verifier 与 optional caller raw-byte full rebuild | 默认禁止真实与 injected dispatch，分别显式 opt-in；固定 not-qualified、只到本地 runtime observation，不进入 Hybrid/Retrieval/Finding；没有真实 multi-Unit live campaign、Truth/P/R 或 provider/runner signature |
| 固定合成 Campaign smoke | 已实现 hash-locked 4-ReviewUnit Campaign、inspect-only default、精确 Campaign/Plan-set/caps acknowledgement、per-Plan atomic marker 和脱敏 terminal summary | 自动化仅 injected transport；本轮未实际联网，不接受任意真实代码 Campaign，不证明模型语义或生产质量 |
