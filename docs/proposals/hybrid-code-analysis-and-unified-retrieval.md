...
status: proposal
implementation: repository_synthetic_live_http_200_outer_invalid_diagnostics_implemented
...
live summary v2 只保留标准化 status/reason、usage、latency、decision counts、artifact IDs，以及
response body hash/size。outer-invalid 时还可携带内容寻址 diagnostic ID、parser contract version、
...
首次 live attempt 发生在上述 diagnostic v1/summary v2 实现之前：固定 transport 收到 HTTP 200，
...
| Repository-synthetic smoke harness | 已实现 package-contained hash-locked 代码单样例、仓库 Prompt/Catalog identity、默认 inspect-only CLI、显式 exact Plan/body controls、同一 state-dir 的本地 marker 与脱敏 summary；自动化测试零网络；一次 live 收到 HTTP 200 但无合法 parsed response/judgment，资产 public/release approval 未证明 |
