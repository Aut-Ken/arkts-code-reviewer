...
LIFECYCLE_EVALUATION_TRUTH_FINGERPRINT = (
    "tag-retrieval-truth:sha256:0e2d0f43c365629cc25d7860694eb289d8c6a2dff119da5aeae85d073e44a04e"
)
...
_EXPECTED_UNADJUDICATED_ADDITIONS = (
    "TR-NET-008",
    "TR-STATE-007",
    "TR-STATE-009",
    "TR-STATE-010",
...
def build_lifecycle_symbol_leaf_comparison(
    ...
    *,
    ...
    _validate_observation_identity(
        candidate,
        role="candidate",
        feature_config_fingerprint=LIFECYCLE_SYMBOL_LEAF_CANDIDATE_FINGERPRINT,
        tags_config_schema_version="tag-config-v3",
        tags_config_version=LIFECYCLE_SYMBOL_LEAF_CANDIDATE_VERSION,
        feature_routing_schema_version=FEATURE_ROUTING_V2_SCHEMA_VERSION,
    )
    _validate_shared_observation_identity(baseline, candidate)

    baseline_rows = _validated_rows_by_case_id(baseline, role="baseline")
    candidate_rows = _validated_rows_by_case_id(candidate, role="candidate")
    ...
    provenance_failures = tuple(
        sorted(
            ...
            if not _has_symbol_leaf_unit_exact_provenance(candidate_rows[case_id])
        )
    )
    trace_provenance_failures = tuple(
        case_id
        for case_id, row in candidate_rows.items()
        if not _has_lifecycle_symbol_leaf_trace_provenance(row)
    )
    ...
    if (
        ...
        or tuple(unadjudicated_cross_target_lifecycle_additions)
        != _EXPECTED_UNADJUDICATED_ADDITIONS
    ...
    safety_blockers = [
        ...
        "ordinary_class_same_name_owner_not_distinguishable",
    ...
    return {
        "schema_version": "lifecycle-symbol-leaf-comparison-v2",
        "evaluation_status": "provisional_candidate_not_activation_evidence",
        ...
        "candidate_lifecycle_target_case_metrics_by_split": metrics_by_split,
...
def _validated_rows_by_case_id(
    ...
    *,
    ...
    for row in rows.values():
        _validate_observation_row(row, role=role)
...
def _validate_observation_row(row: Mapping[str, object], *, role: str) -> None:
    required_fields = {
        *_IMMUTABLE_CASE_FIELDS,
        "actual_exact_tag",
        "actual_routing_tag",
...
def _has_symbol_leaf_unit_exact_provenance(row: Mapping[str, object]) -> bool:
    return _has_symbol_leaf_scope_provenance(row, "unit_exact")


def _has_lifecycle_symbol_leaf_trace_provenance(
    row: Mapping[str, object],
) -> bool:
    ...
    if len(scopes) != len(set(scopes)):
        return False
    return all(
        scope in {"unit_exact", "file_hint"} and _has_symbol_leaf_scope_provenance(row, scope)
        for scope in scopes
    )


def _has_symbol_leaf_scope_provenance(
    row: Mapping[str, object],
    scope: str,
) -> bool:
