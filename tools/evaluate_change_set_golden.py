...
DEFAULT_MANIFEST = REPO_ROOT / "tests" / "golden" / "change_set" / "manifest.json"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate structured ChangeSet v1 against its independent Golden Set."
    )
    ...
    try:
        ...
        print(f"Invalid ChangeSet Golden input: {exc}", file=sys.stderr)
...
def _format_report(report: dict[str, Any]) -> str:
    lines = [
        f"ChangeSet Golden: {report['case_count']} cases",
...
def _guard_json_output(path: Path, golden_root: Path) -> None:
    ...
    if output == root or output.is_relative_to(root):
        raise ValueError("--json-output must stay outside the ChangeSet Golden root")
