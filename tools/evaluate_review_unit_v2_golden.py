...
DEFAULT_MANIFEST = REPO_ROOT / "tests" / "golden" / "review_unit_v2" / "manifest.json"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate ChangeSet-aware ReviewUnit v2 Golden fixtures."
    )
    ...
    try:
        ...
        print(f"Invalid ReviewUnit v2 Golden input: {exc}", file=sys.stderr)
...
def _format_report(report: dict[str, Any]) -> str:
    lines = [
        f"ReviewUnit v2 Golden: {report['case_count']} cases",
    ...
    for case in report["cases"]:
        if not case["matched"]:
            ...
            lines.extend(f"      invariant: {item}" for item in case["invariant_violations"][:5])
    return "\n".join(lines)
...
def _guard_json_output(path: Path, golden_root: Path) -> None:
    ...
    if output == root or output.is_relative_to(root):
        raise ValueError("--json-output must stay outside the ReviewUnit v2 Golden root")
