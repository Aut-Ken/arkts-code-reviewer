...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Seal one independently authored lifecycle review receipt. This command "
...
def _load_draft(path: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise ValueError(f"invalid lifecycle review draft {path}: {exc}") from exc
    if not isinstance(payload, dict):
        raise ValueError("lifecycle review draft must be a JSON object")
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        receipt = LifecycleHoldoutReviewReceipt.model_validate_json(canonical_json(payload))
        validate_lifecycle_holdout_review_receipt(receipt, packet)
    except (OSError, ValueError) as exc:
        print(f"Lifecycle blind review receipt seal failed: {exc}", file=sys.stderr)
        return 2
    print(
        json.dumps(
